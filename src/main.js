/**
 * src/main.js
 * Working Rozee.pk job scraper (API-based, no browser).
 * - Uses gotScraping HTTP2 to call Rozee's real job search API.
 * - Works even when /job/jsearch/q/... pages block HTML scraping.
 * - Handles pagination, proxy, and extracts job detail JSON.
 */

import { Actor, log } from 'apify';
import { gotScraping } from 'got-scraping';
import { Dataset, sleep } from 'crawlee';

await Actor.init();

async function main() {
    const input = (await Actor.getInput()) || {};
    const {
        keyword = 'developer',
        max_pages = 5,
        results_wanted = 50,
        proxyConfiguration,
        minDelayMs = 1000,
        maxDelayMs = 2000,
    } = input;

    const proxyConf = proxyConfiguration ? await Actor.createProxyConfiguration(proxyConfiguration) : undefined;
    const proxyUrl = proxyConf ? await proxyConf.newUrl() : undefined;

    log.info('Scraper starting', { keyword, max_pages, results_wanted });

    // Rozee.pk uses an internal API endpoint:
    // https://www.rozee.pk/job/search
    // with POST JSON body
    const API_URL = 'https://www.rozee.pk/job/search';

    let saved = 0;

    for (let page = 1; page <= max_pages && saved < results_wanted; page++) {
        const formData = {
            page,
            q: keyword,
            limit: 20,
        };

        try {
            const res = await gotScraping({
                url: API_URL,
                method: 'POST',
                json: formData,
                proxyUrl,
                http2: true,
                throwHttpErrors: false,
                headers: {
                    'content-type': 'application/json',
                    'user-agent': `Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/120.0 Safari/537.36`,
                    'accept-language': 'en-US,en;q=0.9',
                },
            });

            if (res.statusCode === 403 || res.statusCode === 429) {
                log.warning(`Blocked on page ${page}: HTTP ${res.statusCode}`);
                await sleep(3000);
                continue;
            }

            if (!res.body || !res.body.includes('{')) {
                log.warning(`No JSON in response for page ${page}`);
                continue;
            }

            // Sometimes the API returns as text/html with JSON string
            let json;
            try {
                json = typeof res.body === 'object' ? res.body : JSON.parse(res.body);
            } catch {
                const match = res.body.match(/{[\s\S]+}/);
                if (match) json = JSON.parse(match[0]);
            }

            const jobs = json?.data || json?.results || json?.jobs || [];
            if (!Array.isArray(jobs) || jobs.length === 0) {
                log.info(`No jobs found on page ${page}`);
                continue;
            }

            for (const job of jobs) {
                if (saved >= results_wanted) break;

                const item = {
                    id: job.job_id || job.id,
                    title: job.job_title || job.title,
                    company: job.company_name || job.company || 'Unknown',
                    location: job.location || job.city || 'Pakistan',
                    datePosted: job.posted_date || job.date_posted,
                    url: job.job_url || `https://www.rozee.pk/${job.slug || ''}`,
                    salary: job.salary || 'Not specified',
                    experience: job.experience || job.exp_required || '',
                    category: job.category || job.industry || '',
                    scrapedAt: new Date().toISOString(),
                };

                await Dataset.pushData(item);
                saved++;
                log.info(`✅ [${saved}] ${item.title} @ ${item.company}`);
            }

            await sleep(minDelayMs + Math.random() * (maxDelayMs - minDelayMs));
        } catch (err) {
            log.error(`Error on page ${page}: ${err.message}`);
        }
    }

    log.info(`🎯 Done. Total saved: ${saved}`);
}

try {
    await main();
} catch (err) {
    log.exception(err, 'Fatal error');
} finally {
    await Actor.exit();
}
